...
import { Loader2, Play, Users, Award, BookOpen, ChevronLeft, ChevronRight, ArrowRight, FileText, GraduationCap, ShieldCheck, Star } from "lucide-react";
import { trpc } from "@/lib/trpc";
import { useRef } from "react";
import WhatsAppButton from "@/components/WhatsAppButton";
import { useLocation, Link } from "wouter";
import ProfessionalsSection from "@/components/ProfessionalsSection";
function CourseCarousel(_a) {
    var courses = _a.courses, title = _a.title;
    ...
    return (<div className="mb-12">
      ...
      <div className="relative group">
        <button onClick={function () { return scroll("left"); }} className="absolute left-0 top-0 bottom-0 z-10 w-12 bg-gradient-to-r from-slate-900/90 to-transparent flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
          <ChevronLeft className="w-8 h-8 text-white"/>
        </button>

        <div ref={scrollRef} className="flex gap-4 overflow-x-auto scrollbar-hide px-4 pb-4" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
          {courses.map(function (course) {
            var priceNum = parseFloat(course.price || "0");
            var slug = course.slug || course.id;
            return (<div key={course.id} className="flex-shrink-0 w-[280px] group/card cursor-pointer" onClick={function () { return setLocation("/curso/".concat(slug)); }}>
                <div className="relative aspect-[16/10] rounded-lg overflow-hidden mb-3 bg-slate-700">
                  {course.coverUrl ? (<img src={course.coverUrl} alt={course.title} className="w-full h-full object-cover group-hover/card:scale-105 transition-transform duration-300"/>) : (<div className="w-full h-full bg-gradient-to-br from-teal-800 to-teal-600 flex items-center justify-center">
        ...
        <button onClick={function () { return scroll("right"); }} className="absolute right-0 top-0 bottom-0 z-10 w-12 bg-gradient-to-l from-slate-900/90 to-transparent flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
...
export default function Home() {
    var _a = useAuth(), user = _a.user, loading = _a.loading, isAuthenticated = _a.isAuthenticated;
    var _b = useLocation(), setLocation = _b[1];
    var _c = trpc.courses.list.useQuery({
        limit: 50,
        offset: 0,
    }), courses = _c.data, coursesLoading = _c.isLoading;
    var freeCourses = trpc.courses.getFreeCourses.useQuery({
        limit: 20,
        offset: 0,
    }).data;
    var articles = trpc.articles.list.useQuery().data;
    return (<div className="min-h-screen bg-slate-900">
      ...
      <nav className="sticky top-0 z-50 bg-slate-900/95 backdrop-blur-md border-b border-slate-800">
        <div className="container max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3 cursor-pointer" onClick={function () { return setLocation("/"); }}>
            <img src="/assets/LOGOMARCA EDUCADQ P FUNDO ESCURO C TURQUESA.png" alt="EducaDQ Logo" className="h-10 w-auto object-contain" onError={function (e) {
            var _a;
            // Fallback if image fails to load
            e.currentTarget.style.display = 'none';
            (_a = e.currentTarget.parentElement.querySelector('.logo-fallback')) === null || _a === void 0 ? void 0 : _a.classList.remove('hidden');
        }}/>
          ...
          <div className="flex items-center gap-3">
            {isAuthenticated ? (<>
                <span className="text-slate-400 text-sm hidden sm:inline">{user === null || user === void 0 ? void 0 : user.name}</span>
                <Button size="sm" className="bg-teal-600 hover:bg-teal-700" onClick={function () {
                if ((user === null || user === void 0 ? void 0 : user.role) === "admin")
                    setLocation("/admin");
                else if ((user === null || user === void 0 ? void 0 : user.role) === "professor")
              ...
              </>) : (<Button size="sm" className="bg-teal-600 hover:bg-teal-700" onClick={function () { return setLocation("/login"); }}>
                Entrar
              </Button>)}
          </div>
        </div>
        ...
        <div className="relative container max-w-7xl mx-auto px-4 py-20 md:py-32">
          <div className="max-w-3xl">
            ...
            <div className="flex flex-wrap gap-4">
              <Button size="lg" className="bg-teal-600 hover:bg-teal-700 text-white px-8" onClick={function () { return setLocation("/cursos"); }}>
                Explorar Cursos <ArrowRight className="w-4 h-4 ml-2"/>
              </Button>
              {!isAuthenticated && (<Button size="lg" variant="outline" className="border-slate-600 text-slate-300 hover:bg-slate-800" onClick={function () { return setLocation("/login"); }}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-16">
            {[
            { icon: BookOpen, label: "Cursos", value: (courses === null || courses === void 0 ? void 0 : courses.length) || 0 },
            { icon: Users, label: "Alunos", value: "500+" },
            { icon: Award, label: "Certificados", value: "200+" },
            { icon: ShieldCheck, label: "Aprovação", value: "95%" },
        ].map(function (stat, i) { return (<div key={i} className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-4 text-center">
      ...
      <section id="courses" className="py-12">
        <div className="container max-w-7xl mx-auto">
          {coursesLoading ? (<div className="flex justify-center py-20">
              <Loader2 className="w-8 h-8 animate-spin text-teal-500"/>
            </div>) : courses && courses.length > 0 ? (<>
              {freeCourses && freeCourses.length > 0 && (<CourseCarousel courses={freeCourses} title="🎓 Cursos Gratuitos - Comece Agora!"/>)}
              <CourseCarousel courses={courses} title="Cursos em Destaque"/>
              {courses.length > 3 && (<CourseCarousel courses={__spreadArray([], courses, true).reverse()} title="Mais Populares"/>)}
      ...
      {/* Articles Section */}
      {articles && articles.length > 0 && (<section className="py-12 bg-slate-800/30">
          <div className="container max-w-7xl mx-auto px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {articles.map(function (article) { return (<Link key={article.id} href={"/artigos/".concat(article.slug)}>
      ...
      <section className="py-16">
        ...
        ].map(function (dep, i) { return (<Card key={i} className="bg-slate-800 border-slate-700">
                <CardContent className="p-6">
      ...
      <section id="about" className="py-16 bg-slate-800/30">
        ...
        ].map(function (item, i) { return (<div key={i} className="p-6 rounded-xl bg-slate-800/50 border border-slate-700/50">
      ...
      <section className="py-16">
        <div className="container max-w-4xl mx-auto px-4 text-center">
          ...
          {!isAuthenticated && (<Button size="lg" className="bg-teal-600 hover:bg-teal-700 text-white px-8" onClick={function () { return setLocation("/login"); }}>
