...
export default function ContentPortal() {
    ...
    var filteredArticles = articles.filter(function (article) {
        var matchesSearch = article.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
            article.excerpt.toLowerCase().includes(searchTerm.toLowerCase());
        var matchesCategory = !selectedCategory || article.category === selectedCategory;
        return matchesSearch && matchesCategory;
    });
    return (<div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 py-12 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          <div className="relative">
            ...
            <Input type="text" placeholder="Buscar artigos..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} className="pl-12 py-3 text-base"/>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1">
            <Card className="sticky top-4">
              ...
              <CardContent className="space-y-2">
                <Button variant={selectedCategory === null ? "default" : "outline"} className="w-full justify-start" onClick={function () { return setSelectedCategory(null); }}>
                  Todas as categorias
                </Button>
                {categories.map(function (category) { return (<Button key={category.id} variant={selectedCategory === category.id ? "default" : "outline"} className="w-full justify-between" onClick={function () { return setSelectedCategory(category.id); }}>
          ...
          <div className="lg:col-span-3 space-y-6">
            {filteredArticles.length > 0 ? (filteredArticles.map(function (article) { return (<Card key={article.id} className="hover:shadow-lg transition-shadow">
                  <CardHeader>
