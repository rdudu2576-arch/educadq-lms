...
export default function PaymentPage() {
    ...
    var _b = useState(null), selectedPayment = _b[0], setSelectedPayment = _b[1];
    if (loading) {
    ...
    // Fetch payments
    var _c = trpc.payments.getStudentPayments.useQuery(undefined, { enabled: !!user }), payments = _c.data, paymentsLoading = _c.isLoading;
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className="container mx-auto px-4 py-12">
        {paymentsLoading ? (<div className="flex justify-center py-12">
            <Loader2 className="w-8 h-8 animate-spin text-cyan-500"/>
          </div>) : payments && Array.isArray(payments) && payments.length > 0 ? (<div className="space-y-6">
            {payments === null || payments === void 0 ? void 0 : payments.map(function (payment) { return (<Card key={payment.id} className="bg-slate-800 border-slate-700 hover:border-cyan-500 transition-colors cursor-pointer" onClick={function () { return setSelectedPayment(selectedPayment === payment.id ? null : payment.id); }}>
                <CardHeader>
                ...
                {selectedPayment === payment.id && (<CardContent className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="w-full h-2 bg-slate-700 rounded-full overflow-hidden">
                        ...
                        width: "".concat((payment.paidInstallments / payment.installmentCount) * 100, "%"),
                    }}/>
                      </div>
                    </div>
                    ...
                    {/* Action button */}
                    {payment.status !== "paid" && (<Button className="w-full bg-cyan-600 hover:bg-cyan-700">
                        Realizar Pagamento
                      </Button>)}
                  </CardContent>)}
              </Card>); })}
