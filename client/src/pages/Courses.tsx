...
export default function Courses() {
  ...
  const { data: courses, isLoading } = trpc.courses.list.useQuery({ limit: 100, offset: 0 });

  const filteredCourses = courses?.filter((course) => {
    const matchesSearch = course.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
      course.description?.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesPrice = !showOnlyFree || parseFloat(course.price || "0") === 0;
  ...
  return (
    <>
      <SEO
        title="Cursos - EducaDQ"
        description="Explore nossos cursos especializados em dependência química, prevenção a recaídas e neurociência."
      />
      <div className="min-h-screen bg-slate-900">
        ...
        <div className="bg-gradient-to-br from-gray-900 via-teal-900 to-gray-900 border-b border-teal-700/30">
          <div className="container max-w-6xl mx-auto px-4 py-4">
            ...
            <button
                onClick={() => setLocation("/", {})}
              className="flex items-center gap-2 text-teal-300 hover:text-teal-200 transition-colors text-sm mt-2"
            >
            ...
            <div className="flex flex-col md:flex-row gap-4 items-start md:items-center">
              ...
              <label className="flex items-center gap-2 text-white cursor-pointer whitespace-nowrap">
                <input
                  ...
                  className="w-4 h-4 rounded border-slate-600 bg-slate-700 text-teal-600 cursor-pointer"
                />
            ...
            {isLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredCourses.map((course) => {
                  ...
                  return (
                    <Card key={course.id} className="bg-slate-800 border-slate-700 hover:border-teal-600 transition-colors cursor-pointer" onClick={() => setLocation(`/curso/${slug}`)}>
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <Button
                              ...
                              onClick={(e) => {
                                ...
                                } else {                                  setLocation(`/checkout/${course.id}`);                              }
                              }}
                            >
