...
export default function StudentDashboard() {
    ...
    var _c = trpc.courses.getStudentCourses.useQuery(undefined, { enabled: !!user }), enrollments = _c.data, enrollmentsLoading = _c.isLoading;
    // Fetch all available courses
    var _d = trpc.courses.list.useQuery({ limit: 50, offset: 0 }, { enabled: !!user }), allCourses = _d.data, coursesLoading = _d.isLoading;
    // Categorize enrollments
    var _e = useMemo(function () {
        if (!enrollments)
            return { inProgress: [], completed: [] };
        return {
            inProgress: enrollments.filter(function (e) { return e.status === "active"; }),
            completed: enrollments.filter(function (e) { return e.status === "completed"; }),
        };
    }, [enrollments]), inProgress = _e.inProgress, completed = _e.completed;
    // Find courses not enrolled in
    var availableCourses = useMemo(function () {
        if (!allCourses || !enrollments)
            return allCourses || [];
        var enrolledCourseIds = new Set(enrollments.map(function (e) { return e.courseId; }));
        return allCourses.filter(function (c) { return !enrolledCourseIds.has(c.id); });
    ...
    if (!user) {
        ...
        <Card className="bg-slate-800 border-slate-700 max-w-md">
          <CardContent className="pt-6 text-center">
            ...
            <Button onClick={function () { return window.location.href = getLoginUrl(); }} className="bg-cyan-600 hover:bg-cyan-700">
              Fazer Login
            </Button>
          </CardContent>
        </Card>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className="bg-slate-800/80 border-b border-slate-700 py-6">
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-3 mb-3">
            <Button onClick={function () { return setLocation("/"); }} variant="ghost" size="sm" className="text-slate-400 hover:text-white">
      ...
      <div className="container mx-auto px-4 py-8">
        {enrollmentsLoading || coursesLoading ? (<div className="flex justify-center py-16">
            <Loader2 className="w-8 h-8 animate-spin text-cyan-500"/>
          </div>) : (<Tabs defaultValue="in-progress" className="space-y-6">
            ...
            <TabsContent value="in-progress">
              {inProgress.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {inProgress.map(function (enrollment) { return (<EnrollmentCard key={enrollment.id} enrollment={enrollment} onContinue={function () { return setLocation("/courses/".concat(enrollment.courseId)); }}/>); })}
                </div>) : (<EmptyState icon={<BookOpen className="w-12 h-12 text-slate-500"/>} title="Nenhum curso em andamento" description="Explore os cursos disponíveis e comece a aprender!" action={<Button onClick={function () { return setLocation("/"); }} className="bg-cyan-600 hover:bg-cyan-700">Explorar Cursos</Button>}/>)}
            </TabsContent>

            {/* Completed Tab */}
            <TabsContent value="completed">
              {completed.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {completed.map(function (enrollment) { return (<Card key={enrollment.id} className="bg-slate-800 border-green-700/30 hover:border-green-600/50 transition-colors">
                      <CardHeader>
                      ...
                      <CardContent className="space-y-3">
                        ...
                        {enrollment.finalGrade && (<p className="text-sm text-slate-400">
                            Nota final: <span className="text-white font-semibold">{enrollment.finalGrade}</span>
                          </p>)}
                        <Button onClick={function () { return setLocation("/courses/".concat(enrollment.courseId)); }} variant="outline" className="w-full border-slate-600 text-slate-300 hover:bg-slate-700">
                          Revisar Curso
                        </Button>
                      </CardContent>
                    </Card>); })}
                </div>) : (<EmptyState icon={<GraduationCap className="w-12 h-12 text-slate-500"/>} title="Nenhum curso concluído" description="Continue estudando para concluir seus cursos!"/>)}
            </TabsContent>

            {/* Available Courses Tab */}
            <TabsContent value="available">
              {availableCourses.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {availableCourses.map(function (course) { return (<Card key={course.id} className="bg-slate-800 border-slate-700 hover:border-cyan-500/50 transition-colors">
                      {course.coverUrl && (<div className="h-40 overflow-hidden rounded-t-lg">
                      ...
                      <CardContent className="space-y-3">
                        ...
                        <Button onClick={function () { return setLocation("/payments?courseId=".concat(course.id)); }} className="w-full bg-cyan-600 hover:bg-cyan-700">
                          <ShoppingCart className="w-4 h-4 mr-2"/> Adquirir Curso
                        </Button>
                      </CardContent>
                    </Card>); })}
                </div>) : (<EmptyState icon={<Star className="w-12 h-12 text-slate-500"/>} title="Você já possui todos os cursos!" description="Parabéns! Continue estudando e acompanhe seu progresso."/>)}
            </TabsContent>
          </Tabs>)}

        {/* Recommendations Section */}
        {availableCourses.length > 0 && (<div className="mt-12">
            <div className="flex items-center gap-2 mb-6">
              <TrendingUp className="w-5 h-5 text-cyan-500"/>
              <h2 className="text-xl font-bold text-white">Cursos Recomendados</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {availableCourses.slice(0, 4).map(function (course) { return (<Card key={course.id} className="bg-slate-800/50 border-slate-700 hover:border-cyan-500/30 transition-colors cursor-pointer" onClick={function () { return setLocation("/payments?courseId=".concat(course.id)); }}>
...
function EnrollmentCard(_a) {
    var enrollment = _a.enrollment, onContinue = _a.onContinue;
    var progressValue = enrollment.progress || 0;
    return (<Card className="bg-slate-800 border-slate-700 hover:border-cyan-500/50 transition-colors">
      <CardHeader>
        <div className="flex items-start justify-between">
          <div>
            <CardTitle className="text-white">Curso #{enrollment.courseId}</CardTitle>
