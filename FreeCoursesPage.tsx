...
export function FreeCoursesPage() {
  ...
  const { data: courses, isLoading } = trpc.courses.getFreeCourses.useQuery({
    limit: 20,
    offset: 0,
  });
        ...
        {courses && courses.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {courses.map((course: any) => (
              <Card key={course.id} className="bg-slate-800 border-slate-700 hover:border-cyan-600 transition-all">
                {course.coverUrl && (
                  <img
                    src={course.coverUrl}
                    alt={course.title}
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    <Button
                      onClick={() => navigate(`/curso/${course.slug || course.id}`)}
