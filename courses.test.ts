...
describe("Courses Router", () => {
  ...
  describe("getBySlug", () => {
    it("should return course with lessons and modules by slug", async () => {
      ...
      vi.mocked(db.getCourseBySlug).mockResolvedValueOnce(mockCourse);
    ...
    it("should throw NOT_FOUND if course does not exist", async () => {
      vi.mocked(db.getCourseBySlug).mockResolvedValueOnce(null);
  ...
  describe("create", () => {
    it("should allow admin to create a course", async () => {
      ...
      vi.mocked(db.createCourse).mockResolvedValueOnce(mockCourse);
  ...
  describe("getStudentCourses", () => {
    it("should return courses for authenticated student", async () => {
      ...
      vi.mocked(db.getStudentCourses).mockResolvedValueOnce(mockCourses);
      ...
      expect(result).toEqual(mockCourses);
      expect(db.getStudentCourses).toHaveBeenCalledWith(2);
    });
...
describe("Lessons Router", () => {
  ...
  describe("addMaterial", () => {
    it("should allow admin to add material to lesson", async () => {
      ...
      vi.mocked(db.addMaterial).mockResolvedValueOnce(mockMaterial);
      ...
      expect(db.addMaterial).toHaveBeenCalled();
    });
  });
  ...
  describe("getMaterialsByLessonId", () => {
    it("should return materials by lesson ID", async () => {
      ...
      vi.mocked(db.getMaterialsByLessonId).mockResolvedValueOnce(
        mockMaterials
      );
      ...
      const result = await caller.lessons.getMaterialsByLessonId({
        lessonId: 1,
      });
  ...
  describe("removeMaterial", () => {
    it("should allow admin to remove material", async () => {
      vi.mocked(db.removeMaterial).mockResolvedValueOnce(true);
      ...
      expect(db.removeMaterial).toHaveBeenCalledWith(1);
    });
  });
});
