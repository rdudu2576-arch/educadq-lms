...
import { Loader2, MapPin, Briefcase, Mail, Phone, MessageCircle, Linkedin, Instagram, Globe } from "lucide-react";
...
export default function ProfessionalProfilePage() {
  const [match, params] = useRoute("/aluno/:slug");
  const slug = params?.slug as string;

  const { data: profile, isLoading } = trpc.ranking.getPublicRanking.useQuery(
    { limit: 1, offset: 0 },
    { enabled: !!slug }
  );

  // Filter profile by slug
  const filteredProfile = profile?.find((p: any) => p.slug === slug);
  ...
  const p = filteredProfile as any;

  return (
    <>
      <SEO
        title={`${p.publicName} - Profissional EducaDQ`}
        description={p.bio || `Perfil profissional de ${p.publicName} na plataforma EducaDQ`}
        ogImage={p.profileImageUrl || undefined}
      />
