...
export const rankingRouter = router({
  ...
  getMyProfile: protectedProcedure.query(async ({ ctx }: any) => {
  ...
  getSubscriptionStatus: protectedProcedure.query(async ({ ctx }: any) => {
  ...
  getAllSubscriptions: protectedProcedure.query(async ({ ctx }: any) => {
  ...
  getIntegrityStatus: protectedProcedure.query(async ({ ctx }: any) => {
    if (ctx.user.role !== "admin") {
      throw new TRPCError({ code: "FORBIDDEN" });
    }
