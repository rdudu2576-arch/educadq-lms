...
export const rankingRouter = router({
  ...
  getPublicRanking: publicProcedure
    ...
    .query(async ({ input }) => {
      return db.getRanking(input.limit);
    }),
  ...
  getSubscriptionHistory: protectedProcedure
    ...
    .query(async ({ ctx }) => {
      return db.getSubscriptionHistory(ctx.user!.id);
    }),
  ...
  confirmSubscription: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const history = await db.getSubscriptionHistory(ctx.user!.id);
      if (!history.length) {
        throw new TRPCError({
          code: "NOT_FOUND",
  ...
  blockUserAccount: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await db.blockUserAccount(input.userId);
