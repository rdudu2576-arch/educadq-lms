...
export const coursesRouter = router({
  ...
  getFreeCourses: publicProcedure
    ...
    .query(async ({ input }: any) => {
      // Get all courses with price = 0
      const { getCourses } = await import("../../infra/db");
      const courses = await getCourses();
      const freeCourses = courses.filter((c: any) => c.price === 0);
      return freeCourses.slice(input.offset || 0, (input.offset || 0) + (input.limit || 10));
    }),
  ...
  getStudentCourses: protectedProcedure.query(async ({ ctx }: any) => {
    if (!ctx.user) {
      throw new TRPCError({
        code: "UNAUTHORIZED",
