...
export default function CourseView({ params }: CourseViewProps) {
  ...
  const { data: progress, refetch: refetchProgress } = trpc.progress.getCourseProgress.useQuery(
    { courseId },
    { enabled: !!user && !isNaN(courseId), retry: false }
  );
  ...
  const { data: assessments } = trpc.assessments.getByCourse.useQuery(
    { courseId },
    { enabled: !!user && !isNaN(courseId) }
  );
  ...
  const completedLessonIds = useMemo(() => {
    return new Set(progress?.progress?.map((p: any) => p.lessonId) || []);
  }, [progress]);
      ...
      <div className="bg-slate-800/80 border-b border-slate-700 py-6">
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-3 mb-3">
            <Button onClick={() => setLocation("/student")} variant="ghost" size="sm" className="text-slate-400 hover:text-white">
              ...
              <CardContent className="p-2">
                <div className="space-y-1 max-h-[60vh] overflow-y-auto">
                  {lessons.map((lesson: any, index: number) => {
                    ...
                    const isLocked = index > 0 && !completedLessonIds.has(lessons[index - 1].id);
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-700">
                    ...
                    {assessments.map((assessment: any) => (
                      <button
                        ...
                        className="w-full text-left p-3 rounded-lg bg-amber-900/20 text-amber-400 hover:bg-amber-900/30 transition-all flex items-center gap-2 text-sm mb-1"
                      >
            ...
            {currentLesson ? (
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {currentLesson.durationMinutes && (
                      <span className="text-xs text-slate-500">{currentLesson.durationMinutes} min</span>
                    )}
                  </div>
                </div>
                ...
                {currentLesson.type === "video" && currentLesson.videoUrl && (() => {
                  const ytId = extractYouTubeId(currentLesson.videoUrl);
                  return ytId ? (
                    ...
                    <CardContent className="pt-6">
                      <div className="text-center py-8">
                        ...
                        {currentLesson.liveUrl ? (
                          <Button
                            onClick={() => window.open(currentLesson.liveUrl!, "_blank")}
                          ...
                          <p className="text-slate-500">Link da aula ainda não disponível.</p>
                        )}
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card className="bg-slate-800 border-slate-700">
                    <CardContent className="pt-6">
                      <div className="text-center py-4">
                        ...
                        {currentLesson.content && (
                          <p className="text-slate-400 mb-4">{currentLesson.content}</p>
                        )}
                      </div>
                    </CardContent>
                  </Card>
